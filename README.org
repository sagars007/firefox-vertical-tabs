...
** How to install
- Go to [[about:config][about:config]], search for ~toolkit.legacyUserProfileCustomizations.stylesheets~ and set it to ~true~.
...
- Go to [[about:addons][about:addons]], select Tab Center Reborn, go to Preferences and set:
  + Animations: on.
